...
import { avatarsDir, latestBackup, schemaVersion, storageOrigin, startDurablePersistence } from "./src/database/connection.mjs";
...
app.get("/developer-tools.html",requireCapability("admin_panel"),(req,res)=>res.sendFile(path.join(publicDir,"developer-tools.html")));
app.get(["/admin","/admin.html"],requireCapability("admin_panel"),(req,res)=>res.sendFile(path.join(publicDir,req.gameIndexAccess.capabilities.includes("creator_control")?"admin.html":"developer-tools.html")));
...
app.get(["/creator-studio","/creator-studio.html"],requireCapability("creator_studio_edit"),(req,res)=>res.redirect(302,"/universe-builder.html?mode=page-authoring"));
...
app.use(express.static(publicDir,{extensions:["html"],maxAge:0,etag:true,lastModified:true,setHeaders(res,file){
  ...
  const hotEntry=/\/(?:index|games|game|profile|user|social|showcase|update-log|report-bug|admin|animation-editor|cinematic-test-lab|music-manager|image-library|universe-builder|login|settings|ai-control|ai-flow)\.html$/i.test(normalized)||/\/(?:ai|builder-labels|delivery-0992|developer-tools|mobile-i2|shell-0986|cinematic-0991-hf1|gameindex-0991-i1|animation-editor|animation-runtime|cinematic-test-lab|profile|public-profile|profile-settings|social|update-log|report-bug|experience-engine|game-experience-manager|app|games|game|music-manager|image-library|image-crop-editor|universe-builder|visual-grounding-099i1|visual-grounding-099i2|universe-interaction-engine-099i2|universe-runtime-099|interactive-runtime-099|auth|settings)\.js$/i.test(normalized)||/\/(?:stability-0992-i1|delivery-0992|gameindex-mobile-i2|gameindex-09915|gameindex-0986|gameindex-0991-hf1|gameindex-0991-i1|cinematic-0991-hf1|animation-runtime|animation-editor|universe-builder-v3|experience-engine|game|image-manager-hf1|final-foundation-099)\.css$/i.test(normalized);
