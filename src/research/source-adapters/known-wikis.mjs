import {registeredSources} from '../registered-sources.mjs';
...
export function knownWikiSources(game,family){return [...(game?.id?registeredSources(game.id).filter(x=>x.family===family):[]),...(sources[String(game?.slug||'').toLowerCase()]?.[family]||[]).map(url=>({url}))].map(x=>({...x,title:game?.nome||game?.name||''}));}
