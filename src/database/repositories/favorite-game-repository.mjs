...
function favoritesReady(){
  try{return Boolean(db.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name='user_favorite_games'`).get());}
  catch{return false;}
}
function requireFavoritesReady(){
  if(favoritesReady())return;
  const error=new Error("Jogos Favoritos está sendo atualizado. Tente novamente após a atualização do GameIndex.");
  error.code="FAVORITE_GAMES_NOT_READY";error.status=503;throw error;
}
...
export function isGameFavorite(userId,gameId){
  if(!favoritesReady())return false;
