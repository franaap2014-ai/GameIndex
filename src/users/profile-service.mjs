...
export function buildProfile(userId){
  ...
  return {user:{...user,staffRole:access.staffRole,badge:access.badge,staffSuspended:access.suspended},socialProfile:social,preferences:getPreference(userId),subscription:getSubscription(userId),counts:{followedGames:followedCount(userId),savedKnowledge:savedKnowledgeCount(userId),creatorOutputs:creatorOutputCount(userId),activity:activityCount(userId),followers:followerCount(userId),following:followingCount(userId),friends:friendCount(userId)},followedGames:listFollowedGames(userId,{limit:24}),savedKnowledge:listSavedKnowledge(userId,{limit:24}),creatorHistory:listCreatorOutputs(userId,{limit:12}),recentActivity:listActivity(userId,{limit:20}),followers:listFollowers(userId,{limit:12}),following:listFollowing(userId,{limit:12}),friends:listFriends(userId,{limit:12})};
}
...
export function buildPublicProfile({username,userId,viewerId=null}={}){
  ...
  return {user:{id:user.id,displayName:social.displayName||user.displayName,createdAt:user.createdAt,badge:access.badge,staffRole:access.staffRole},socialProfile:publicSocial,counts:{followers:followerCount(user.id),following:followingCount(user.id),friends:friendCount(user.id),followedGames:followedCount(user.id)},followedGames:listFollowedGames(user.id,{limit:12}),relationship:relationshipState(viewerId,user.id)};
}
