...
export function setupFirstAdmin(req,res,input={}){
  ...
  try{
    if(!state.configured)throw new Error("A configuração inicial ainda não foi habilitada no Azure.");
    ...
    db.prepare(`INSERT INTO staff_role_assignments(user_id,role,suspended,assigned_by,reason,created_at,updated_at) VALUES(?,'CREATOR',0,?,'Secure first owner setup',?,?) ON CONFLICT(user_id) DO UPDATE SET role='CREATOR',suspended=0,updated_at=excluded.updated_at`).run(userId,userId,nowIso(),nowIso());
