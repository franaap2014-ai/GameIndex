(()=>{
  ...
  if(!document.querySelector('link[data-gi0992-delivery]')){const deliveryStyle=document.createElement('link');deliveryStyle.rel='stylesheet';deliveryStyle.href='/css/gameindex-0992.css?v=0992';deliveryStyle.dataset.gi0992Delivery='1';document.head.appendChild(deliveryStyle);}
  if(!document.querySelector('script[data-gi0992-delivery]')){const deliveryScript=document.createElement('script');deliveryScript.src='/js/delivery-0992.js?v=0992';deliveryScript.defer=true;deliveryScript.dataset.gi0992Delivery='1';document.head.appendChild(deliveryScript);}
})();
const GV={
  ...
  async transitionTheme(nextState={},options={}){
    ...
    const oldAccent=getComputedStyle(document.documentElement).getPropertyValue("--rb-accent").trim()||"#eef3f8";
    ...
    const themeAccent=theme=>({"free-dark":"#eef3f8","free-light":"#252b33","pro-green":"#63e69a","tester-blue":"#58a6ff","dev-red":"#ff5968","dev-green":"#63e69a","dev-blue":"#54a8ff","creator-tech":"#e7bd5b"}[theme]||oldAccent);
    const newAccent=themeAccent(targetTheme);
    const overlay=document.createElement("div");overlay.className="gi-theme-power-transition";overlay.setAttribute("role","presentation");overlay.style.setProperty("--gi-old-accent",oldAccent);overlay.style.setProperty("--gi-new-accent",newAccent);overlay.style.setProperty("--gi-old-cable",oldAccent);overlay.style.setProperty("--gi-new-cable",newAccent);overlay.dataset.sourceTheme=sourceTheme;overlay.dataset.targetTheme=targetTheme;
    ...
    try{
      ...
      overlay.classList.add("gi-hf1-energized");await new Promise(r=>setTimeout(r,460));if(this._themeTransitionToken!==token)return cleanup();
      ensureApply();overlay.classList.add("gi-hf1-booting");await new Promise(r=>setTimeout(r,620));
...
async function initShell(){mountGlobalUi();GV.applyBrand({tier:localStorage.getItem("gv_tier")||"FREE",selectedTheme:localStorage.getItem("gv_theme")||"FREE_DARK"});installImageFailureRecovery();const translationsReady=GV.loadTranslations();const header=document.getElementById("siteHeader"),syncHeader=()=>header?.classList.toggle("scrolled",scrollY>24);syncHeader();addEventListener("scroll",syncHeader,{passive:true});const trigger=document.getElementById("menuTrigger"),drawer=document.getElementById("sideDrawer"),overlay=document.getElementById("drawerOverlay"),close=document.getElementById("drawerClose");let previousFocus=null;const isOpen=()=>drawer?.classList.contains("open");function closeDrawer(){drawer?.classList.remove("open");overlay?.classList.remove("show");drawer?.setAttribute("aria-hidden","true");trigger?.setAttribute("aria-expanded","false");document.body.classList.remove("drawer-open");previousFocus?.focus?.();}trigger?.addEventListener("click",()=>{if(isOpen())return closeDrawer();previousFocus=document.activeElement;drawer?.classList.add("open");overlay?.classList.add("show");drawer?.setAttribute("aria-hidden","false");trigger?.setAttribute("aria-expanded","true");document.body.classList.add("drawer-open");});close?.addEventListener("click",closeDrawer);overlay?.addEventListener("click",closeDrawer);drawer?.querySelectorAll("a[href]").forEach(a=>a.addEventListener("click",closeDrawer));document.addEventListener("keydown",e=>{if(e.key==="Escape"){if(isOpen())closeDrawer();document.getElementById("giMusicPopover")?.setAttribute("hidden","");}if(!isOpen()||e.key!=="Tab")return;const f=focusables(drawer);if(!f.length)return;const first=f[0],last=f.at(-1);if(e.shiftKey&&document.activeElement===first){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}});document.getElementById("globalSearchForm")?.addEventListener("submit",e=>{e.preventDefault();const q=document.getElementById("globalSearch")?.value.trim();if(q)location.href=`/results.html?q=${encodeURIComponent(q)}`;});
  ...
  try{const data=await GV.api("/api/beta0986/access"),access=data.access||{},caps=new Set(access.capabilities||[]),panel=document.getElementById("adminPanelMenu"),panelButton=document.getElementById("adminPanelToggle"),links=document.getElementById("adminPanelLinks"),canPanel=["creator_control","universe_build","image_management","music_management","animation_edit","cinematic_test","manage_staff_roles","ai_diagnostics","database_explorer","deployment_monitor","bug_triage"].some(cap=>caps.has(cap));if(panel)panel.hidden=!canPanel;if(canPanel){links?.querySelectorAll("[data-cap]").forEach(a=>a.hidden=!caps.has(a.dataset.cap));links?.querySelectorAll("[data-cap-any]").forEach(a=>{const required=String(a.dataset.capAny||"").split(",").map(x=>x.trim()).filter(Boolean);a.hidden=!required.some(cap=>caps.has(cap));});const stored=localStorage.getItem("gi_admin_panel_open")==="1";links.hidden=!stored;panelButton.setAttribute("aria-expanded",String(stored));panelButton.addEventListener("click",()=>{const open=links.hidden;links.hidden=!open;panelButton.setAttribute("aria-expanded",String(open));localStorage.setItem("gi_admin_panel_open",open?"1":"0");});document.documentElement.dataset.roleTheme=String(access.roleTheme||"FREE").toLowerCase();}}catch{}
...
(function gameIndexMusic0987(){
  ...
  const mutedKey='gi_audio_muted',enabledKey='gi_audio_enabled',pausedKey='gi_audio_paused',volumeKey='gi_audio_volume',zeroMuteKey='gi_audio_zero_muted',robloxExperienceKey='gi_experience_roblox';
  ...
  const isMuted=()=>localStorage.getItem(mutedKey)==='1',isEnabled=()=>sessionStorage.getItem(enabledKey)!=='0'&&sessionStorage.getItem(pausedKey)!=='1';
  ...
  function render(){
    ...
    if(status){if(lastError==='UNAVAILABLE')status.textContent='Música indisponível no momento.';else if(musicState==='BLOCKED_BY_AUTOPLAY')status.textContent='A música começa após sua primeira interação.';else if(has)status.textContent=`Loop ativo · ${loopCount} reinício${loopCount===1?'':'s'}`;else status.textContent='';}
  ...
  function loadIframeApi(){
    ...
    apiPromise=new Promise((resolve,reject)=>{
      ...
      if(existing)return;
      const script=document.createElement('script');script.src='https://www.youtube.com/iframe_api';script.async=true;script.dataset.giYoutubeIframeApi='1';script.onerror=()=>{apiPromise=null;reject(new Error('YOUTUBE_IFRAME_API_LOAD_FAILED'));};document.head.appendChild(script);
    });
    return apiPromise;
  }
  function playerNode(){
    ...
    playerHost=document.createElement('div');playerHost.id='giYouTubeMusicPlayer';playerHost.setAttribute('aria-hidden','true');host.replaceChildren(playerHost);return playerHost;
  }
  ...
  function switchVariant(variant){
    const next=variant==='roblox-og'?'roblox-og':'main';activeVariant=next;if(currentContext?.slug==='roblox')localStorage.setItem(robloxExperienceKey,next);sessionStorage.setItem(enabledKey,'1');sessionStorage.removeItem(pausedKey);lastError='';resolvedProfile=null;loopCount=0;
  ...
  function togglePlay(){lastError='';const profile=current();if(!profile?.youtubeVideoId){musicState='IDLE';render();return false;}if(playerState===1){sessionStorage.setItem(enabledKey,'0');sessionStorage.setItem(pausedKey,'1');savePlayback();try{player.pauseVideo();}catch{}playerState=2;musicState='PAUSED';render();return true;}sessionStorage.setItem(enabledKey,'1');sessionStorage.removeItem(pausedKey);localStorage.setItem(mutedKey,'0');localStorage.removeItem(zeroMuteKey);if(!player){ensurePlayer(profile.youtubeVideoId,{play:true});return true;}try{player.unMute();player.setVolume(effectiveVolume());player.playVideo();}catch{}musicState='READY';render();return true;}
  function preview(videoId,defaultVolume=30){resolvedProfile={youtubeVideoId:String(videoId),defaultVolume:Number.isFinite(Number(defaultVolume))?Number(defaultVolume):30,preview:true,label:'Preview'};activeVariant='main';sessionStorage.setItem(enabledKey,'1');sessionStorage.removeItem(pausedKey);localStorage.setItem(mutedKey,'0');localStorage.removeItem(zeroMuteKey);popoverOpen=true;ensurePlayer(resolvedProfile.youtubeVideoId,{play:true});render();return true;}
  ...
  function bind(){
    if(sessionStorage.getItem(enabledKey)===null)sessionStorage.setItem(enabledKey,'1');
    setContext(contextFromLocation());
    const resumeAfterGesture=()=>{if(musicState!=='BLOCKED_BY_AUTOPLAY'||!isEnabled()||!current()?.youtubeVideoId)return;try{player?.unMute();player?.setVolume(effectiveVolume());player?.playVideo();musicState='READY';render();}catch{}};
    addEventListener('pointerdown',resumeAfterGesture,{capture:true,passive:true});
    addEventListener('keydown',resumeAfterGesture,{capture:true});
