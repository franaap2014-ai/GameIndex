(()=>{
  ...
  for(const [tag,url] of [['link','/css/delivery-0992.css?v=0992'],['script','/js/delivery-0992.js?v=0992']]){const el=document.createElement(tag);if(tag==='link'){el.rel='stylesheet';el.href=url;}else{el.src=url;el.defer=true;}document.head.appendChild(el);}
...
const GV={
  lang:(window.GIStorage?.local||localStorage).getItem("gv_lang")||"pt-BR",translations:{},auth:null,
  async api(url,options={}){const controller=new AbortController(),timeout=options.timeout||20000,timer=setTimeout(()=>controller.abort(),timeout);try{const response=await fetch(url,{credentials:"same-origin",...options,signal:options.signal||controller.signal});let data={};try{data=await response.json();}catch{}if(!response.ok){const rawMessage=data.error?.message||data.erro||`Erro ${response.status}`;const publicMessage=response.status>=500||/SQLITE|SQLSTATE|constraint failed|no such (?:table|column)|stack trace|ECONN|ENOENT|TypeError|ReferenceError/i.test(rawMessage)?"Não foi possível concluir a operação. Tente novamente em instantes.":rawMessage;const error=new Error(publicMessage);error.status=response.status;error.data=data;throw error;}return data;}catch(error){if(error.name==="AbortError")throw new Error(this.t("error.timeout","A operação demorou demais. Tente novamente."));throw error;}finally{clearTimeout(timer);}},
  ...
  async setLanguage(lang,{persist=true,reload=false}={}){const supported=new Set(["pt-BR","en-US","es-ES"]);this.lang=supported.has(lang)?lang:"pt-BR";if(persist)(window.GIStorage?.local||localStorage).setItem("gv_lang",this.lang);await this.loadTranslations();if(reload)location.reload();},
  ...
  async transitionTheme(nextState={},options={}){
    ...
    if(reduced){document.documentElement.classList.add("gi0991-theme-fade");await new Promise(r=>setTimeout(r,100));if(this._themeTransitionToken!==token)return;apply();await new Promise(r=>setTimeout(r,150));document.documentElement.classList.remove("gi0991-theme-fade");return;}
    const oldAccent=getComputedStyle(document.documentElement).getPropertyValue("--rb-accent").trim()||"#ffffff";
    const sourceTheme=String(document.documentElement.dataset.theme||(window.GIStorage?.local||localStorage).getItem("gv_theme")||"free-dark").toLowerCase();
    const targetTheme=String(nextState.selectedTheme||nextState.theme||sourceTheme).toLowerCase().replaceAll("_","-");
    const root=document.documentElement,previousTheme=root.dataset.theme;
    let targetAccent=oldAccent;try{root.dataset.theme=targetTheme;targetAccent=getComputedStyle(root).getPropertyValue('--rb-accent').trim()||oldAccent;}finally{if(previousTheme===undefined)delete root.dataset.theme;else root.dataset.theme=previousTheme;}
    const overlay=document.createElement("div");overlay.className="gi-theme-power-transition";overlay.setAttribute("role","presentation");overlay.style.setProperty("--gi-old-accent",oldAccent);overlay.style.setProperty("--gi-new-accent",targetAccent);overlay.style.setProperty("--gi-old-cable",oldAccent);overlay.style.setProperty("--gi-new-cable",targetAccent);overlay.dataset.sourceTheme=sourceTheme;overlay.dataset.targetTheme=targetTheme;
    ...
    let applied=false,cleaned=false;const ensureApply=()=>{if(!applied){applied=true;apply();}};const cleanup=()=>{if(cleaned)return;cleaned=true;overlay.remove();if(!document.querySelector(".gi-theme-power-transition"))document.documentElement.classList.remove("gi0991-theme-switching");};
    ...
    try{
      ...
      overlay.classList.add("gi-hf1-transfer");await new Promise(r=>setTimeout(r,380));if(this._themeTransitionToken!==token)return cleanup();overlay.classList.add("gi-hf1-energized");ensureApply();
...
<div class="drawer-overlay" id="drawerOverlay" aria-hidden="true"></div><aside class="side-drawer" id="sideDrawer" role="dialog" aria-modal="true" aria-hidden="true" tabindex="-1"><div class="drawer-panel"><div class="drawer-head"><a class="brand compact gi-brand-lockup" href="/" aria-label="Game Index — Home"><span class="brand-mark gi-brand-mark" data-gi-brand-mark="1"><b aria-hidden="true">GI</b></span><span class="gi-brand-copy"><strong data-gi-brand-word>GAME INDEX</strong><small class="gi-brand-classification" data-gi-brand-classification hidden></small></span></a><button class="drawer-close" id="drawerClose" type="button">×</button></div><div class="drawer-profile" id="drawerProfile"><div class="avatar gi-auto-avatar"><img alt="Visitante" src="${GV.autoAvatar({seed:'guest',name:'Visitante',classification:'FREE'})}"></div><div><strong>Visitante</strong><span>Beta 0.992</span></div></div><nav class="drawer-nav">
...
<div id="adminPanelMenu" class="admin-panel-menu" hidden><button type="button" id="adminPanelToggle" aria-expanded="false"><span>◆</span><strong data-i18n="menu.adminPanel">ADMIN PANEL</strong><span class="chevron">⌄</span></button><div id="adminPanelLinks" class="admin-panel-links" hidden><span class="admin-panel-group-label">OVERVIEW</span><a data-cap="admin_panel" href="/admin.html" data-i18n="admin.panel">Painel ADM</a><span class="admin-panel-group-label">CONTENT & GAMES</span><a data-cap="universe_build" href="/universe-builder.html" data-i18n="admin.universe">Universe Builder</a><a data-cap="image_management" href="/image-library.html" data-i18n="admin.images">Image Manager</a><a data-cap="music_management" href="/music-manager" data-i18n="admin.music">Music Manager</a><a data-cap="creator_control" href="/game-experience-manager.html">Personalization</a><span class="admin-panel-group-label">CINEMATICS</span><a data-cap="animation_edit" href="/animation-editor.html">Cinematic Editor</a><a data-cap="cinematic_test" href="/cinematic-test-lab.html">Cinematic Test Lab</a><span class="admin-panel-group-label">SYSTEM & DIAGNOSTICS</span><a data-cap="manage_staff_roles" href="/admin.html#connections">Admin Connections</a><a data-cap="ai_diagnostics" href="/ai-control">AI Control</a><a data-cap="ai_diagnostics" href="/ai-flow">AI Flow</a><a data-cap="database_explorer" href="/database-explorer.html">Database</a><a data-cap="deployment_monitor" href="/deployment-monitor.html">Deployment</a><a data-cap="bug_triage" href="/bug-tracker.html">Bug Tracker</a></div></div></nav><div class="drawer-footer"><span class="live-dot"></span><span>Beta 0.992</span></div></div></aside></div>`;}
...
(()=>{
  ...
  script.src="/js/gameindex-0991-i1.js?v=09915hf3";
  script.defer=true;
...
(function gameIndexMusic0987(){
  ...
  const isMuted=()=>(window.GIStorage?.local||localStorage).getItem(mutedKey)==='1',isEnabled=()=>(window.GIStorage?.session||sessionStorage).getItem(enabledKey)!=='0';
  function userVolume(){const raw=(window.GIStorage?.local||localStorage).getItem(volumeKey);if(raw===null||raw==='')return null;const n=Number(raw);return Number.isFinite(n)?Math.max(0,Math.min(100,Math.round(n))):null;}
  const trackId=p=>p?.enabled===false?'':p?.audioUrl?'audio:'+p.audioUrl:p?.youtubeVideoId||'';
  ...
  function selectedRobloxVariant(){const raw=(window.GIStorage?.local||localStorage).getItem(robloxExperienceKey);return raw==='roblox-og'?'roblox-og':'main';}
  function trackLabel(){
    ...
    if(CHILD_LABELS[slug])return trackId(current())?`Música · ${CHILD_LABELS[slug]}`:`${CHILD_LABELS[slug]} · sem música configurada`;
    return trackId(current())?'Música desta página':'Nenhuma música configurada';
  }
  ...
  function render(){
    const b=button(),p=popover(),has=Boolean(trackId(current())),muted=isMuted(),playing=playerState===1;
    ...
    if(p){p.hidden=!popoverOpen;p.dataset.unavailable=lastError?'1':'0';}if(surface())surface().hidden=Boolean(lastError)||String(activeVideoId).startsWith('audio:');
    emit();
  }
  function loadIframeApi(){
    if(window.YT?.Player)return Promise.resolve(window.YT);
    if(apiPromise)return apiPromise;
    apiPromise=new Promise((resolve,reject)=>{const timeout=setTimeout(()=>reject(new Error('YOUTUBE_API_TIMEOUT')),12000);const done=value=>{clearTimeout(timeout);resolve(value);};
      const previous=window.onYouTubeIframeAPIReady;
      window.onYouTubeIframeAPIReady=()=>{try{previous?.();}catch{};window.YT?.Player?done(window.YT):reject(new Error('YOUTUBE_IFRAME_API_UNAVAILABLE'));};
    ...
    }).catch(error=>{apiPromise=null;document.querySelector('script[data-gi-youtube-iframe-api="1"]')?.remove();throw error;});
    return apiPromise;
  }
  function playerNode(){
  ...
  function savePlayback(){try{if(player&&activeVideoId&&!current()?.preview)(window.GIStorage?.session||sessionStorage).setItem('gi_audio_resume',JSON.stringify({videoId:activeVideoId,time:Math.max(0,Number(player.getCurrentTime())||0),at:Date.now()}));}catch{}}
  function resumeTime(id){try{const r=JSON.parse((window.GIStorage?.session||sessionStorage).getItem('gi_audio_resume')||'null');return r?.videoId===id&&Date.now()-r.at<28800000&&Number.isFinite(r.time)?Math.max(0,r.time):0;}catch{return 0;}}
  ...
  function onPlayerError(){musicState='UNAVAILABLE';lastError='UNAVAILABLE';playerState=-1;try{player?.destroy?.();}catch{}player=null;playerHost=null;const fallback=current()?.fallback;if(fallback?.youtubeVideoId&&!fallbackAttempted){fallbackAttempted=true;resolvedProfile=fallback;ensurePlayer(fallback.youtubeVideoId,{play:isEnabled()});}render();}
  let fallbackAttempted=false;
  async function ensureAudio(id,{play=true}={}){
    try{player?.destroy?.();}catch{}player=null;playerHost=null;const audio=new Audio(id.slice(6));audio.loop=true;audio.preload='metadata';const blocked=()=>{musicState='BLOCKED_BY_AUTOPLAY';render();};
    const adapter={getCurrentTime:()=>audio.currentTime,pauseVideo:()=>audio.pause(),playVideo:()=>{audio.play().catch(error=>{if(error.name==='NotAllowedError')blocked();else onPlayerError();});},stopVideo:()=>{audio.pause();audio.currentTime=0;},setVolume:v=>{audio.volume=v/100;},mute:()=>{audio.muted=true;},unMute:()=>{audio.muted=false;},setLoop:()=>{audio.loop=true;},seekTo:t=>{audio.currentTime=t;},destroy:()=>{audio.pause();audio.removeAttribute('src');audio.load();}};
    player=adapter;activeVideoId=id;lastError='';playerInstances=1;audio.addEventListener('loadedmetadata',()=>{if(player!==adapter)return;const t=resumeTime(id);if(t<audio.duration)audio.currentTime=t;syncPlayer({play});});audio.addEventListener('playing',()=>{if(player===adapter)onPlayerState({data:1});});audio.addEventListener('pause',()=>{if(player===adapter)onPlayerState({data:2});});audio.addEventListener('error',()=>{if(player===adapter)onPlayerError();});syncPlayer({play});render();return true;
  }
  async function ensurePlayer(id,{play=true}={}){
    if(player&&activeVideoId===String(id)){syncPlayer({play});return true;}if(String(id).startsWith('audio:/audio/'))return ensureAudio(id,{play});if(String(activeVideoId).startsWith('audio:')){try{player?.destroy?.();}catch{}player=null;playerHost=null;}
    ...
    playerPromise=(async()=>{
      ...
      if(!activeVideoId||String(activeVideoId).startsWith('audio:'))return null;const initialId=activeVideoId;
      player=new YT.Player(node,{width:'240',height:'135',videoId:initialId,playerVars:{start:Math.floor(resumeTime(initialId)),autoplay:play&&isEnabled()?1:0,playsinline:1,controls:1,loop:1,playlist:initialId,rel:0,origin:location.origin},events:{onReady:e=>{if(e?.target&&e.target!==player)return;playerInstances=1;try{player.setLoop?.(true);}catch{}syncPlayer({play});setTimeout(()=>syncPlayer({play}),250);render();},onStateChange:e=>{if(!e?.target||e.target===player)onPlayerState(e);},onError:e=>{if(!e?.target||e.target===player)onPlayerError();},onAutoplayBlocked:e=>{if(e?.target&&e.target!==player)return;musicState="BLOCKED_BY_AUTOPLAY";render();}}});
      return player;
    })().catch(error=>{onPlayerError();console.warn('[GameIndex Music]',error?.message||error);return null;}).finally(()=>{playerPromise=null;});
  ...
  async function fetchMusic(url){const c=new AbortController(),timer=setTimeout(()=>c.abort(),10000);try{return await fetch(url,{credentials:'same-origin',signal:c.signal});}finally{clearTimeout(timer);}}
  async function loadContext(){
    ...
    try{
      ...
      const requests=[fetchMusic(mainUrl)];
      if(ctx.type==='game'&&String(ctx.slug).toLowerCase()==='roblox')requests.push(fetchMusic(`/api/games/${encodeURIComponent(ctx.slug)}/music/alt/roblox-og`));
      ...
      currentProfile=main?.profile||null;alternateProfile=alt?.profile||null;resolvedProfile=null;activeVariant=ctx.type==='game'&&ctx.slug==='roblox'?selectedRobloxVariant():'main';musicState=trackId(directProfile())?'READY':'IDLE';lastError='';loopCount=0;
      if(isEnabled()&&trackId(directProfile()))await ensurePlayer(trackId(directProfile()),{play:true});else if(!trackId(directProfile()))stopPlayback();render();
    }catch{if(token!==loadingToken)return;currentProfile=alternateProfile=resolvedProfile=null;stopPlayback();onPlayerError();}
  }
  function setContext(next){if(contextKey(next)===contextKey(currentContext))return;savePlayback();fallbackAttempted=false;stopPlayback();activeVideoId='';currentContext=next;activeVariant=next?.slug==='roblox'?selectedRobloxVariant():'main';resolvedProfile=null;loadContext();}
  function switchVariant(variant){
    const next=variant==='roblox-og'?'roblox-og':'main';activeVariant=next;if(currentContext?.slug==='roblox')(window.GIStorage?.local||localStorage).setItem(robloxExperienceKey,next);(window.GIStorage?.session||sessionStorage).setItem(enabledKey,'1');lastError='';resolvedProfile=null;loopCount=0;
    const profile=directProfile();if(trackId(profile))ensurePlayer(trackId(profile),{play:true});else stopPlayback();render();window.dispatchEvent(new CustomEvent('gameindex:music-variant',{detail:{variant:activeVariant,hasTrack:Boolean(trackId(profile))}}));return true;
  }
  function applyResolvedProfile(profile,{variant=activeVariant,context=currentContext}={}){
    if(context)currentContext=context;activeVariant=variant==='roblox-og'?'roblox-og':'main';resolvedProfile=trackId(profile)?profile:null;lastError='';loopCount=0;
    if(trackId(resolvedProfile)&&isEnabled())ensurePlayer(trackId(resolvedProfile),{play:true});else if(!trackId(resolvedProfile)&&!trackId(directProfile()))stopPlayback();render();return true;
  }
  function setVolume(value){const raw=Number(value),v=Math.max(0,Math.min(100,Math.round(Number.isFinite(raw)?raw:30)));(window.GIStorage?.local||localStorage).setItem(volumeKey,String(v));try{player?.setVolume(v);}catch{}if(v===0){(window.GIStorage?.local||localStorage).setItem(mutedKey,'1');(window.GIStorage?.local||localStorage).setItem(zeroMuteKey,'1');try{player?.mute();}catch{}}else if((window.GIStorage?.local||localStorage).getItem(zeroMuteKey)==='1'){(window.GIStorage?.local||localStorage).removeItem(zeroMuteKey);(window.GIStorage?.local||localStorage).setItem(mutedKey,'0');(window.GIStorage?.session||sessionStorage).setItem(enabledKey,'1');try{player?.unMute();player?.playVideo();}catch{}}render();return v;}
  function toggleMute(){const next=!isMuted();(window.GIStorage?.local||localStorage).removeItem(zeroMuteKey);(window.GIStorage?.local||localStorage).setItem(mutedKey,next?'1':'0');if(next){try{player?.mute();}catch{}}else{(window.GIStorage?.session||sessionStorage).setItem(enabledKey,'1');if(trackId(current())&&!player)ensurePlayer(trackId(current()),{play:true});try{player?.unMute();player?.setVolume(effectiveVolume());player?.playVideo();}catch{}}render();}
  function togglePlay(){(window.GIStorage?.session||sessionStorage).setItem(enabledKey,'1');(window.GIStorage?.local||localStorage).setItem(mutedKey,'0');(window.GIStorage?.local||localStorage).removeItem(zeroMuteKey);lastError='';const profile=current();if(!trackId(profile)){musicState='IDLE';render();return false;}if(!player){ensurePlayer(trackId(profile),{play:true});return true;}if(playerState===1){(window.GIStorage?.session||sessionStorage).setItem(enabledKey,'0');savePlayback();try{player.pauseVideo();}catch{}playerState=2;musicState='PAUSED';}else{try{player.unMute();player.setVolume(effectiveVolume());player.playVideo();}catch{}musicState='READY';}render();return true;}
  function preview(videoId,defaultVolume=30){resolvedProfile={youtubeVideoId:String(videoId),defaultVolume:Number.isFinite(Number(defaultVolume))?Number(defaultVolume):30,preview:true,label:'Preview'};activeVariant='main';(window.GIStorage?.session||sessionStorage).setItem(enabledKey,'1');(window.GIStorage?.local||localStorage).setItem(mutedKey,'0');(window.GIStorage?.local||localStorage).removeItem(zeroMuteKey);popoverOpen=true;ensurePlayer(trackId(resolvedProfile),{play:true});render();return true;}
  ...
  function bind(){
    ...
    addEventListener('pagehide',()=>{savePlayback();try{player?.pauseVideo();}catch{}});
    addEventListener('pageshow',()=>{if(player&&isEnabled())syncPlayer({play:true});});
    const resumeAfterGesture=e=>{if(e.type==='keydown'&&(e.key!=='Enter'&&e.key!==' '))return;if(e.target?.closest?.('#giMusicPlay,#giMusicMute,#giVolume'))return;if(isEnabled()&&musicState==='BLOCKED_BY_AUTOPLAY')syncPlayer({play:true});};document.addEventListener('pointerdown',resumeAfterGesture);document.addEventListener('keydown',resumeAfterGesture);
