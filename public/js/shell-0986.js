const GV={
  ...
  async transitionTheme(nextState={},options={}){
    ...
    const previousState={tier:document.documentElement.dataset.tier||localStorage.getItem("gv_tier")||"FREE",selectedTheme:(document.documentElement.dataset.theme||localStorage.getItem("gv_theme")||"free-dark").toUpperCase().replaceAll("-","_")};
    ...
    if(reduced){document.documentElement.classList.add("gi0991-theme-fade");await new Promise(r=>setTimeout(r,90));apply();await new Promise(r=>setTimeout(r,150));document.documentElement.classList.remove("gi0991-theme-fade");return;}
    const oldBrand=this.resolveBrand(previousState),oldAccent=getComputedStyle(document.documentElement).getPropertyValue("--rb-accent").trim()||oldBrand.primary||"#ffffff";
    const overlay=document.createElement("div");overlay.className="gi-theme-power-transition";overlay.setAttribute("role","presentation");overlay.style.setProperty("--gi-old-accent",oldAccent);overlay.style.setProperty("--gi-new-accent",oldAccent);overlay.innerHTML=`<div class="gi0991-flicker gi0991-flicker-in"></div><div class="gi0991-blackout"></div><div class="gi0991-power-scene"><div class="gi0991-socket"><i></i><i></i></div><div class="gi0991-cable-wrap"><div class="gi0991-cable"></div><div class="gi0991-plug"><span class="gi0991-pin a"></span><span class="gi0991-pin b"></span></div></div><div class="gi0991-tech-rise">${[0,1,2,3,4,5].map(i=>`<i class="t${i}"></i>`).join("")}</div><div class="gi0991-energy-pulse"></div></div><div class="gi0991-flicker gi0991-flicker-out"></div>`;
    ...
    const safety=setTimeout(()=>{if(this._themeTransitionToken===token)ensureApply();cleanup();},3500);
    try{
      await new Promise(r=>setTimeout(r,430));if(this._themeTransitionToken!==token)return cleanup();
      overlay.classList.add("gi0991-unplug");await new Promise(r=>setTimeout(r,470));if(this._themeTransitionToken!==token)return cleanup();
      ensureApply();const newAccent=getComputedStyle(document.documentElement).getPropertyValue("--rb-accent").trim()||"#ffffff";overlay.style.setProperty("--gi-new-accent",newAccent);overlay.classList.add("gi0991-new-power");await new Promise(r=>setTimeout(r,380));
      if(this._themeTransitionToken!==token)return cleanup();overlay.classList.add("gi0991-reconnect");await new Promise(r=>setTimeout(r,620));
      if(this._themeTransitionToken!==token)return cleanup();overlay.classList.add("gi0991-boot");await new Promise(r=>setTimeout(r,520));
...
<div class="drawer-overlay" id="drawerOverlay" aria-hidden="true"></div><aside class="side-drawer" id="sideDrawer" role="dialog" aria-modal="true" aria-hidden="true" tabindex="-1"><div class="drawer-panel"><div class="drawer-head"><a class="brand compact gi-brand-lockup" href="/"><span class="brand-mark gi-brand-mark" data-gi-brand-mark="1"><i></i><b>G</b></span><span class="gi-brand-copy"><strong data-gi-brand-word>GAME INDEX</strong><small class="gi-brand-classification" data-gi-brand-classification hidden></small></span></a><button class="drawer-close" id="drawerClose" type="button">×</button></div><div class="drawer-profile" id="drawerProfile"><div class="avatar gi-auto-avatar"><img alt="Visitante" src="${GV.autoAvatar({seed:'guest',name:'Visitante',classification:'FREE'})}"></div><div><strong>Visitante</strong><span>Beta 0.99</span></div></div><nav class="drawer-nav">
