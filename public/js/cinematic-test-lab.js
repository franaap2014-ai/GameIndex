...
document.addEventListener("DOMContentLoaded",()=>{$("ctlSourceType").onchange=updateRows;$("ctlViewport").onchange=()=>{$("ctlStage").dataset.viewport=$("ctlViewport").value;};$("ctlPlay").onclick=()=>{const state=S.controller?.getState?.();if(state&&state.time>0&&state.time<state.duration&&JSON.stringify(selectedRequest())===JSON.stringify(S.lastRequest)){S.controller.play();return;}S.sequenceToken++;preview(selectedRequest()).catch(e=>msg(e.message,"error"));};$("ctlReplay").onclick=()=>{S.sequenceToken++;preview(S.lastRequest||selectedRequest()).catch(e=>msg(e.message,"error"));};$("ctlPause").onclick=()=>S.controller?.pause();$("ctlStop").onclick=()=>{S.sequenceToken++;S.controller?.pause();S.controller?.seek(0);};$("ctlAddSequence").onclick=()=>{S.sequence.push(selectedRequest());renderSequence();};$("ctlRunSequence").onclick=()=>runSequence().catch(e=>msg(e.message,"error"));$("ctlClearSequence").onclick=()=>{S.sequenceToken++;S.sequence=[];clearController();renderSequence();};$("ctlSequence").onclick=e=>{const b=e.target.closest("[data-remove-sequence]");if(!b)return;S.sequence.splice(Number(b.dataset.removeSequence),1);renderSequence();};updateRows();boot();});
})();