...
const S={catalog:null,games:[],controller:null,lastRequest:null,sequence:[],sequenceToken:0};
...
function clearController(){S.controller?.destroy?.();S.controller=null;}
...
async function preview(request,{awaitEnd=false,sequenceToken=S.sequenceToken}={}){
  clearController();S.lastRequest=request;
  ...
  document.documentElement.dataset.ctlReduced=request.reducedMotion?"1":"0";
  ...
  S.controller.play(0);
  if(awaitEnd)await waitForPlayback(S.controller,runtime,sequenceToken);
}
