...
try{
  ...
  assert.equal(h.status,"ok");assert.equal(h.version,"0.991");assert.equal(h.release,"BETA_0_991_HF1_IDENTITY_RESTORATION");assert.equal(h.schema,41);
  const apiHealth=await request(`${base}/api/health`);const api=await apiHealth.json();assert.equal(api.version,"0.991");assert.equal(api.release,"BETA_0_991_HF1_IDENTITY_RESTORATION");
  ...
  await request(`${base}/games.html`);await request(`${base}/login.html`);await request(`${base}/universe-builder.html`,401);
  ...
  console.log(JSON.stringify({ok:true,release:"0.991-HF1",schema:41,health:true,apiHealth:true,home:true,games:true,protectedUniverseBuilder:true,productionStart:true},null,2));
