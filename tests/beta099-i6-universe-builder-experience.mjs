...
assert.ok(['gameindex-beta-099-i6-universe-builder-experience','gameindex-beta-099-i6-hf1-build-reliability','gameindex-beta-099-i6-hf2-launch-visual-rebrand','gameindex-beta-0991-full-experience','gameindex-beta-0991-hf1-identity-restoration'].includes(pkg.name));
assert.ok(['0.99.6','0.99.6-1','0.99.6-2','0.991.0','0.991.1'].includes(pkg.version));
assert.ok(['node tests/beta099-i6-universe-builder-experience.mjs','node tests/beta099-i6-hf1-build-reliability.mjs','node tests/beta099-i6-hf2-launch-rebrand.mjs','node tests/beta0991-full-experience.mjs','node tests/beta0991-hf1-identity-restoration.mjs'].includes(pkg.scripts.test));
assert.equal(pkg.engines.node,'>=22.13');

const release=read('src/config/release-099i6.mjs');
assert.match(release,/PUBLIC_VERSION="(?:0\.99|0\.991)"/);assert.match(release,/INTERNAL_RELEASE="(?:0\.99-I6(?:-HF[12])?|0\.991(?:-HF1)?)"/);assert.match(release,/TARGET_SCHEMA=(?:39|40|41)/);assert.match(release,/LOCAL_FIRST_NO_API_KEY/);
