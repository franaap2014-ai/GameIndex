...
try{
 ...
 await test('manual pause remains paused after visibility changes',()=>{settings.events.onStateChange({data:1});context.window.GameIndexMusic.togglePlay();assert.equal(storage.getItem('gi_audio_enabled'),'0');assert.equal(pauses,1);const before=plays;listeners.visibilitychange();assert.equal(plays,before);});
