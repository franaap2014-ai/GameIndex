...
assert.match(connection,/GAMEINDEX_RENDER_PERSISTENT_DISK/);
...
assert.match(release,/INTERNAL_RELEASE="0\.991-I1-HF1"/);
assert.match(release,/TARGET_SCHEMA=43/);
assert.match(release,/BETA_0_991_I1_HF1_NEON_PERSISTENCE/);

assert.match(env,/DATABASE_URL=/);
assert.match(env,/GAMEINDEX_REMOTE_SNAPSHOT_KEY=production/);
assert.match(env,/GAMEINDEX_TARGET_SCHEMA=43/);
assert.match(server,/await startDurablePersistence/);
assert.match(server,/NEON PERSISTENCE ONLINE/);
assert.match(admin,/NEON_REMOTE_SQLITE_SNAPSHOT|Neon/);

assert.equal(pkg.version,"0.991.3");
assert.equal(pkg.scripts.test,"node tests/beta0991-i1-hf1-neon-persistence.mjs");
